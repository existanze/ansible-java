...
# Existanze Artifactory Certificate
openssl s_client -connect central.existanze.gr:443 </dev/null |  sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > central.existanze.gr.cert
keytool -trustcacerts -keystore $KEYSTORE -storepass changeit -noprompt -importcert -alias central.existanze.gr -file central.existanze.gr.cert

rm -f letsencryptauthorityx1.der letsencryptauthorityx2.der lets-encrypt-x1-cross-signed.der lets-encrypt-x2-cross-signed.der lets-encrypt-x3-cross-signed.der lets-encrypt-x4-cross-signed.der central.existanze.gr.cert
